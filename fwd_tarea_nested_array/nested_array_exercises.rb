def blank_seating_chart(number_of_rows, seats_per_row)
  ...
  # change with it
  arrayNew= Array.new(number_of_rows,Array.new(seats_per_row))
end
puts blank_seating_chart(2,3)
...
matrix2=['Bo', nil, 'Kim']
result = add_another_row(matrix,matrix2)
...
resp=delete_seat_from_row(matrix3,0, 1)
resp.each {|row| p row }
